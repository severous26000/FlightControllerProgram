...
uint8_t* read_tx_adress_register() {
	...
	cs_low();
	HAL_SPI_TransmitReceive(NRF24L01P_SPI, &command, &status, 1, 2000);
	HAL_SPI_Receive(NRF24L01P_SPI, &read_val, 5, 2000);
	cs_high();

	//Begin of trusted code
	for (int i = 0; i < 5; i++) {
		HAL_UART_Transmit(NRF24L01P_UART, &read_val[i], 1, 1000);
	}
	//End of trusted code
...
static uint8_t write_tx_adress_register(uint8_t value[5]) {
	uint8_t command = NRF24L01P_CMD_W_REGISTER | NRF24L01P_REG_TX_ADDR;
	uint8_t status;

	cs_low();
	HAL_SPI_TransmitReceive(NRF24L01P_SPI, &command, &status, 1, 2000);
	HAL_SPI_Transmit(NRF24L01P_SPI, value, 5, 2000);
	//HAL_UART_Transmit(NRF24L01P_UART, value, 5, 1000);
	cs_high();

	return status;
}

uint8_t* read_rx_adress_register() {
	uint8_t command = NRF24L01P_CMD_R_REGISTER | NRF24L01P_REG_RX_ADDR_P0;
	uint8_t status;
	uint8_t read_val[5];
...
static uint8_t write_rx_adress_register(uint8_t value[5]) {
...
void NRF24L01P_Rx_Init(channel MHz, air_data_rate bps) {
	...
	uint8_t address[5] = { 0x11, 0x22, 0x33, 0x44, 0x55 };
	nRF24L01_Set_Rx_Addr(address);
...
void nrf24l01p_reset() {
	...
	write_register(NRF24L01P_REG_RF_SETUP, 0x0F);
	write_register(NRF24L01P_REG_STATUS, 0x0E); //Modified
...
void nRF24L01_Set_Rx_Addr(uint8_t addr[5]) {
	write_rx_adress_register(addr);
}

void nRF24L01_Set_Tx_Addr(uint8_t addr[5]) {
	write_tx_adress_register(addr);
}
